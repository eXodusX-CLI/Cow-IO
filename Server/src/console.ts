...
dispatcher.register(
  literal("addObject").then(
    argument("type", string()).executes((context) => {
      let type = context.getArgument("type", String);
      let thisPlayer = context.getSource() as Player;
      let game = getGame();

      if (game) {
        let player = game.state.players.find(
          (player) => player.id == thisPlayer.id
        );
      ...
      (context) => {
        ...
        if (game) {
          if (thisPlayer) {
            switch (variant) {
              case "dark":
                thisPlayer.selectedWeapon === thisPlayer.weapon ? thisPlayer.primaryWeaponVariant = WeaponVariant.Dark : thisPlayer.secondaryWeaponVariant = WeaponVariant.Dark;
                break;

              case "emerald":
                thisPlayer.selectedWeapon === thisPlayer.weapon ? thisPlayer.primaryWeaponVariant = WeaponVariant.Emerald : thisPlayer.secondaryWeaponVariant = WeaponVariant.Emerald;
                break;
        ...
        (context) => {
          ...
          if (game) {
            ...
            if (player) {
              switch (variant) {
                case "dark":
                  player.selectedWeapon === player.weapon ? player.primaryWeaponVariant = WeaponVariant.Dark : player.secondaryWeaponVariant = WeaponVariant.Dark;
                  break;
                case "ruby":
...
export { startConsole, log, runCommand, logMethod };
