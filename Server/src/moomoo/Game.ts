...
import { getWeaponDamage, getWeaponAttackDetails, getItemCost, getPlaceable, PrimaryWeapons, getWeaponGatherAmount, getPrerequisiteItem, getGroupID, Weapons, getPrerequisiteWeapon, getWeaponSpeedMultiplier, getStructureDamage, getPPS, isRangedWeapon, getProjectileType, getWeaponLength, getRecoil, SecondaryWeapons } from "../items/items";
...
export default class Game {
  ...
  async initDatabase() {
    this.db = await lowdb(new FileAsync<DBSchema>('./.data/db.json'));
  ...
  killPlayer(player: Player) {
    ...
    let playerObjects = this.state.gameObjects.filter(gameObj => gameObj.ownerSID === player.id)
    let isThereSpawnpad: boolean = false
    playerObjects.forEach(gameObj => {if (gameObj.spawnPoint) isThereSpawnpad = true})
    playerObjects.forEach(gameObj => {
      if (isThereSpawnpad) return;
    ...
    this.state.projectiles.forEach(projectile => {
      ...
      this.state.getPlayersNearProjectile(projectile).forEach(player => {
        ...
        if (Physics.collideProjectilePlayer(projectile, player) && player.id != projectile.ownerSID) {
          if (owner)
            this.damageFrom(player, owner, owner.selectedWeapon, projectile.damage, false);
  ...
  damageFrom(to: Player, from: Player, currentWeapon: number, dmg: number, direct = true) {
    let owner = this.state.players.find(player => player.id == from.id);
    if (owner?.selectedWeapon != currentWeapon) return;
    let packetFactory = PacketFactory.getInstance();
        ...
        if (now - player.lastHitTime >= player.getWeaponHitTime()) {
          ...
          if (isRangedWeapon(player.selectedWeapon)) {
            ...
            for (let hitPlayer of hitPlayers) {
              ...
              this.damageFrom(hitPlayer, player, player.selectedWeapon, dmg);

              if ([WeaponVariant.Ruby, WeaponVariant.Emerald].includes(weaponVariant)) {
              ...
              if (weaponVariant === WeaponVariant.Emerald && !player.dead && player.health < 100) {
                let healAmount = (dmg?dmg:0)*0.30
                player.health = Math.min(player.health+healAmount, 100)
                packetFactory.serializePacket(
                  new Packet(
                    PacketType.HEALTH_CHANGE,
                    [player.location.x, player.location.y, Math.round(-healAmount), 1]
                  )
                )
              }

              if (weaponVariant === WeaponVariant.Dark && !hitPlayer.dead) {
    ...
    switch (packet.type) {
      case PacketType.SPAWN:
        ...
        if (
          ...
          if (!player || (player && player.dead)) {
            ...
            let playerObjects = this.state.gameObjects.filter(gameObj => gameObj.ownerSID == newPlayer.id)
            let spawnPad = playerObjects.filter(gameObj => gameObj.spawnPoint == true)[0]
            newPlayer.location = (spawnPad) ? spawnPad.location : randomPos(14400, 14400);
            if (spawnPad) this.state.removeGameObject(spawnPad)
            ...
            newPlayer.food = 100000;
            newPlayer.points = 100000;
            newPlayer.stone = 100000;
            newPlayer.wood = 100000;
        ...
        if (client.player) {
          ...
          if (isWeapon) {
            ...
            if (client.player.weapon == packet.data[0]) {
              if (client.player.selectedWeapon != client.player.weapon) client.player.lastHitTime = 0;
              client.player.selectedWeapon = client.player.weapon;
            } else if (client.player.secondaryWeapon == packet.data[0]) {
              if (client.player.selectedWeapon != client.player.secondaryWeapon) client.player.lastHitTime = 0;
      ...
      case PacketType.SELECT_UPGRADE:
        ...
        if (client.player) {
          ...
          let newWeapons = new Array();
          newWeapons.push(client.player.weapon);

          if (![-1].includes(client.player.secondaryWeapon)) newWeapons.push(client.player.secondaryWeapon);
