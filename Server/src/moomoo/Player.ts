...
export default class Player extends Entity {
  ...
  public bleedSpd = 5;
  public bleedSpdAmt = 0;
  public maxBleedSpdAmt = -1;
  ...
  public set primaryWeaponExp(value) {
    if (value >= 20000) this.primaryWeaponVariant = WeaponVariant.Dark;
    else if (value >= 16000) this.primaryWeaponVariant = WeaponVariant.Emerald;
  ...
  public set secondaryWeaponExp(value) {
    ...
    else*/ if (value >= 16000) this.secondaryWeaponVariant = WeaponVariant.Emerald;
  ...
  public healOverTime(healAmount: number = 0) {
    let packetFactory = PacketFactory.getInstance();
    if (healAmount != 0 && this.health < 100) {
      this.client?.socket.send(
        packetFactory.serializePacket(
          new Packet(
            PacketType.HEALTH_CHANGE,
            [this.location.x, this.location.y, -Math.min(100 - this.health, healAmount), 1]
          )
        )
      );
